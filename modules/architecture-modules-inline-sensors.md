...
- Secondary LLM call cost is attributed to the parent run via [Cost Attribution]({{ site.baseurl }}{% link modules/architecture-modules-cost-attribution.md %})
...
- [Quality Gates]({{ site.baseurl }}{% link modules/architecture-modules-quality-gates.md %}) — same sensors, but run *after* the agent finishes
- [MCP Tool Governance]({{ site.baseurl }}{% link modules/architecture-modules-mcp-tool-governance.md %}) — sensors are Dandori-published MCP tools
- [Use Case Flow 2 — Multi-phase DAG]({{ site.baseurl }}{% link modules/architecture-use-cases.md %}#flow-2-multi-phase-feature-dag)
