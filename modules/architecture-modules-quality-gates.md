...
- Distinct from [Inline Sensors]({{ site.baseurl }}{% link modules/architecture-modules-inline-sensors.md %}) which run *during* the agent's run

## See also

- [Inline Sensors]({{ site.baseurl }}{% link modules/architecture-modules-inline-sensors.md %}) — same sensors but called by the agent mid-run for self-correction
- [Cross-agent Analytics]({{ site.baseurl }}{% link modules/architecture-modules-cross-agent-analytics.md %}) — consumes quality scores for trend lines
- [Use Case Flow 1 — Jira → PR → approval]({{ site.baseurl }}{% link modules/architecture-use-cases.md %}#flow-1-jira-issue--agent-run--pr-with-audit)
